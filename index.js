...
function buildMongoUri() {
  ...
  if (!uri) {
    throw new Error(
      'Missing MONGODB_URI. Put it in .env (local) and Vercel Env Vars (production).'
    );
  }
  return uri;
}
...
/* =========================
   Admin seed
   ========================= */
const SEED_ADMIN_EMAIL = safeStr(
  process.env.SEED_ADMIN_EMAIL || 'nazmul.hasan.palash2000@gmail.com'
).trim();
...
async function initOnce() {
  ...
  _initPromise = (async () => {
    // Firebase init (handles Vercel env JSON correctly)
    initFirebaseAdmin();
    ...
    const customerOrdersCollection = _db.collection('Customer_orders');
    const adminsCollection = _db.collection('Admins');
    ...
    _collections = { productsCollection, customerOrdersCollection, adminsCollection };
    return _collections;
  })();
...
/* =========================
   Ensure DB ready for all routes
   ========================= */
...
app.get('/products', async (req, res) => {
  try {
    const { productsCollection } = await initOnce();
...
app.post('/products', requireAuth, async (req, res) => {
  try {
    const { productsCollection, adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);

    // run admin check inside route
...
app.post('/customer-orders', requireAuth, async (req, res) => {
  try {
    const { productsCollection, customerOrdersCollection } = await initOnce();
...
app.get('/customer-orders', requireAuth, async (req, res) => {
  try {
    const { customerOrdersCollection, adminsCollection } = await initOnce();
...
app.patch('/customer-orders/:id', requireAuth, async (req, res) => {
  try {
    const { customerOrdersCollection, adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);
...
app.put('/customer-orders/:id', requireAuth, async (req, res) => {
  try {
    const { customerOrdersCollection, adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);
...
app.delete('/customer-orders/:id', requireAuth, async (req, res) => {
  try {
    const { customerOrdersCollection, adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);
...
app.get('/admins', requireAuth, async (req, res) => {
  try {
    const { adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);
...
app.delete('/admins/:id', requireAuth, async (req, res) => {
  try {
    const { adminsCollection } = await initOnce();
    const requireAdmin = makeRequireAdmin(adminsCollection);
